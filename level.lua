...
Level.new = function()
    ...
    function self.levelManager(vaisseaux, asteroids, gameSound)
        ...
        if (self.levelNumber == 1) then
            ...
            gameSound = Assets.sounds.level1 or love.audio.newSource("music/BlueNavi-Starcade.mp3", "stream")
            MAX_ASTEROIDS= 1
        end
        if (self.levelNumber == 2) then
            EspacePng = love.graphics.newImage("backgroud/Purple_Nebula_01-512x512.png")
            gameSound = Assets.sounds.level2 or love.audio.newSource("music/Jaunter-Reset.mp3", "stream")
            MAX_ASTEROIDS= 2
        end
        if (self.levelNumber == 3) then
            EspacePng = love.graphics.newImage("backgroud/Purple_Nebula_04-512x512.png")
            gameSound = Assets.sounds.level3 or love.audio.newSource("music/KarolPiczak-LesChampsEtoiles.mp3", "stream")
            MAX_ASTEROIDS= 3
        end
        if (self.levelNumber == 4) then
            EspacePng = love.graphics.newImage("backgroud/Blue_Nebula_08-512x512.png")
            gameSound = Assets.sounds.level4 or love.audio.newSource("music/Kubbi-Ember-04Cascade.mp3", "stream")
            MAX_ASTEROIDS= 4
        end
        if (self.levelNumber == 5) then
            EspacePng = love.graphics.newImage("backgroud/Green_Nebula_07-512x512.png")
            gameSound = Assets.sounds.level5 or love.audio.newSource("music/PunchDeck-ICantStop.mp3", "stream")
            MAX_ASTEROIDS= 5
        end
